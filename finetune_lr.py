...
def main(params):
    ...
    train_history_path = get_ft_train_history_path(output_dir).replace('.csv', '_lr.csv')
    loss_history_path = get_ft_loss_history_path(output_dir).replace('.csv', '_lr.csv')
    valid_history_path = get_ft_valid_history_path(output_dir).replace('.csv', '_lr.csv')
    test_history_path = get_ft_test_history_path(output_dir).replace('.csv', '_lr.csv')
    ...
    for episode in range(n_episodes):
        ...
        opt_params = []
        opt_params.append({'params': head.parameters(), 'lr': 0.01, 'momentum' : 0.9, 'dampening' : 0.9, 'weight_decay' : 0.001})
        opt_params.append({'params': body.parameters(), 'lr': 0.03, 'momentum' : 0.9, 'dampening' : 0.9, 'weight_decay' : 0.001})
