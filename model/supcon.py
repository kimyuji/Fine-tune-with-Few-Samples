...
class SupConLoss(nn.Module):
	...
	def forward(self, features, labels=None, mask=None):
		...
		# 	self.temperature)
		anchor_dot_contrast = torch.div(
			cosine_similarity(anchor_feature.unsqueeze(1), contrast_feature.unsqueeze(0)), # (2N, 2N)
			self.temperature)
...
class FT_SupConLoss(nn.Module):
	def __init__(self, temperature=5, contrast_mode='all',
					base_temperature=5):
		super(FT_SupConLoss, self).__init__()
		self.temperature = temperature
		self.contrast_mode = contrast_mode
		self.base_temperature = base_temperature

	def forward(self, features, labels=None, mask=None): # (N, f_dim)
		device = torch.device('cuda')
