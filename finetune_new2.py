...
def main(params):
    ...
    if params.ft_augmentation and params.ft_with_clean:
        n_data = n_data*2
    ...
    for episode in range(n_episodes):
        ...
        if params.ft_augmentation and params.ft_with_clean:
            y_support = torch.concat([y_support, y_support])
        ...
        for epoch in range(n_epoch):
            ...
            if not use_fixed_features:  # load data every epoch
                ...
                if params.ft_with_clean or params.ft_clean_test:
                    ...
                    if params.ft_with_clean:
                        x_support = torch.concat([x_support_clean, x_support])

            total_loss = 0
            correct = 0
            indices = np.random.permutation(n_data) # shuffle 5 * 5 or 1 * 5

            aug_bool = True
            if params.ft_scheduler_end is not None:
            ...
            with torch.no_grad():
                ...
                # if (params.ft_cutmix or params.ft_mixup) and params.ft_with_clean:
                #     f_support_clean = body.forward_features(x_support, params.ft_features)
                #     f_support_aug = torch.concat([f_support_aug, f_support_clean])
            ...
            for i in range(support_batches):
                ...
                if (params.ft_mixup or params.ft_cutmix) and params.ft_with_clean: # concat original self
