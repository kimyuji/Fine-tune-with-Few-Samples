...
def main(params):
    ...
    test_history_path = test_history_path.replace(".csv", "_TTA_v2.csv")
    train_history_path = train_history_path.replace(".csv", "_TTA_v2.csv")
            ...
            for i in range(support_batches):
                ...
                if use_fixed_features:
                    ...
                    for transformer in transformers: # custom transforms or e.g. tta.aliases.d4_transform() 
                        # augment image
                        x_support_augmented = transformer.augment_image(x_support[batch_indices])
                        
                        # pass to model
                        f_support = body.forward_features(x_support_augmented, params.ft_features)
                        p_support = head(f_support) 
                    ...
                    #     masks.append(deaug_label)
                        p_support_tta.append(p_support.unsqueeze(2))
                        
                    # # reduce results as you want, e.g mean/max/min
                    p_support_tta = torch.concat(p_support_tta, axis=2)
                    p_support_tta_mean = torch.mean(p_support_tta, dim=2)

                correct += torch.eq(y_batch, p_support_tta_mean.argmax(dim=1)).sum()

                if aug_bool and mix_bool:
                    loss = criterion(p_support_tta_mean, y_batch) * lam + criterion(p_support_tta_mean, y_shuffled_batch) * (1. - lam)
                else:
                    loss = criterion(p_support_tta_mean, y_batch)
