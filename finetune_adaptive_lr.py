...
def main(params):
    ...
    output_dir = output_dir.replace('full', upt_dir)
    ...
    for episode in range(n_episodes):
        ...
        if 'stem' not in params.upt_blocks:
            for n, p in body.named_parameters():
                if n.startswith('{}.0'.format(backbone_name)) or n.startswith('{}.1'.format(backbone_name)):
                    p.requires_grad = False
                    
        if 's1' not in params.upt_blocks:
            for n, p in body.named_parameters():
                if n.startswith('{}.4'.format(backbone_name)):
                    p.requires_grad = False
                    
        if 's2' not in params.upt_blocks:
            for n, p in body.named_parameters():
                if n.startswith('{}.5'.format(backbone_name)):
                    p.requires_grad = False
                    
        if 's3' not in params.upt_blocks:
            for n, p in body.named_parameters():
                if n.startswith('{}.6'.format(backbone_name)):
                    p.requires_grad = False
                    
        if 's4' not in params.upt_blocks:
            for n, p in body.named_parameters():
                if n.startswith('{}.7'.format(backbone_name)):
                    p.requires_grad = False
        ...
        for epoch in range(n_epoch):
            ...
            if 'stem' not in params.upt_blocks:
                for n, p in body.named_parameters():
                    if n.startswith('{}.0'.format(backbone_name)) or n.startswith('{}.1'.format(backbone_name)):
                        body.backbone.trunk[0].eval()
                        body.backbone.trunk[1].eval()

            if 's1' not in params.upt_blocks:
                for n, p in body.named_parameters():
                    if n.startswith('{}.4'.format(backbone_name)):
                        body.backbone.trunk[4].eval()

            if 's2' not in params.upt_blocks:
                for n, p in body.named_parameters():
                    if n.startswith('{}.5'.format(backbone_name)):
                        body.backbone.trunk[5].eval()

            if 's3' not in params.upt_blocks:
                for n, p in body.named_parameters():
                    if n.startswith('{}.6'.format(backbone_name)):
                        body.backbone.trunk[6].eval()

            if 's4' not in params.upt_blocks:
                for n, p in body.named_parameters():
                    if n.startswith('{}.7'.format(backbone_name)):
                        body.backbone.trunk[7].eval()
