...
def main(params):
    ...
    output_dir = output_dir.replace("experiment/perplexity/resnet10_simclr_LS_default", "ordering/analysis3/effect_ordering")
    ...
    acc_history_path = os.path.join(output_dir, 'acc_history.csv')
    ...
    df_acc = pd.DataFrame(None, index=list(range(1, n_episodes + 1)),
                            columns=[e + 1 for e in range(5)])
    ...
    for episode in range(n_episodes):
        ...
        # index_base = [i for i in range(0, 5)]
        # indices_list = [list(i) for i in itertools.permutations(index_base)]
        # indices_list = [index_base]

        # analysis2 : effect_ordering

        # 1shot
        indices_list = [[0,1,2,3,4], [0,1,2,4,3], [0,1,3,4,2], [0,2,3,4,1] ,[1,2,3,4,0]]
        # 5shot
        # indices_list = [np.random.permutation(w * s) for i in range(100)] # fix for all epoch
        ...
        for indice in indices_list:
            ...
            head_state =  torch.load('./classifier_init/random.pt')
            head.load_state_dict(copy.deepcopy(head_state))
            #torch.save(head.state_dict(), './classifier_init/random.pt')
            ...
            for epoch in range(n_epoch):
                ...
                if print_epoch_logs and (epoch + 1) % 10 == 0:
                    fmt = 'Epoch {:03d}: Test ACC={:6.3f}'
                    print(fmt.format(epoch + 1, test_acc))
            ...
            test_acc = torch.eq(y_query, p_query.argmax(dim=1)).sum() / (w * q) # 75개의 test data에 대해 test
            acc_history.append(test_acc.item()) # same episode, diff trials
            
        df_acc.loc[episode + 1] = acc_history
        ...
        #print(fmt.format(episode, test_acc_history[-1] * 100))
    ...
    df_acc.to_csv(acc_history_path)
    # df_test.to_csv(test_history_path)
    # df_loss.to_csv(loss_history_path)
    # df_grad.to_csv(grad_history_path)
