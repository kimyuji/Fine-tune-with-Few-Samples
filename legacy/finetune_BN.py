...
def main(params):
    ...
    output_dir = output_dir.replace('full', 'bn')
...
#         # seperate for learning rate
#         no_body = ["fc", "linear"]
#         bn_names = ["BN", "LayerNorm.weight"]
...
#         ]
        ...
        for epoch in range(n_epoch):
            if epoch < 50:
                bn_params = list(map(lambda x: x[1],list(filter(lambda kv: 'BN' in kv[0], body.named_parameters()))))
                body_params = list(map(lambda x: x[1],list(filter(lambda kv: 'BN' not in kv[0], body.named_parameters()))))
                optimizer = torch.optim.SGD([{'params': body_params}, {'params': bn_params}, {'params': head.parameters(), 'lr': 0.01}], lr=0.0, momentum=0.9, dampening=0.9, weight_decay=0.001)
                
            else:
                bn_params = list(map(lambda x: x[1],list(filter(lambda kv: 'BN' in kv[0], body.named_parameters()))))
                body_params = list(map(lambda x: x[1],list(filter(lambda kv: 'BN' not in kv[0], body.named_parameters()))))
                optimizer = torch.optim.SGD([{'params': body_params}, {'params': bn_params, 'lr': 0.01}, {'params': head.parameters()}], lr=0.0, momentum=0.9, dampening=0.9, weight_decay=0.001)
            
            # if epoch < 50:
            #     opt_params = []

            #     # opt_params.append({'params': itertools.chain(*bn_params), 'lr': params.body_lr, 'momentum' : 0.9, 'dampening' : 0.9, 'weight_decay' : 0.001})
            #     # opt_params.append({'params': head.parameters(), 'lr': params.head_lr, 'momentum' : 0.9, 'dampening' : 0.9, 'weight_decay' : 0.001})
            #     opt_params.append(body_parameters)
            #     opt_params.append(body_bn_parameters)
            #     opt_params.append(head_parameters)
            #     optimizer = torch.optim.SGD(body_bn_parameters+body_parameters+head_parameters)
            # else:
            #     opt_params = []
