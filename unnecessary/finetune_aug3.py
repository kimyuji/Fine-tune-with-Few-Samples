...
def main(params):
    ...
    if params.ft_cutmix:
        train_history_path = train_history_path.replace('.csv', '_cutmix3.csv')
        test_history_path = test_history_path.replace('.csv', '_cutmix3.csv')
    ...
    for episode in range(n_episodes):
        ...
        for epoch in range(n_epoch):
            ...
            for i in range(support_batches):
                ...
                y = torch.concat([y_support[batch_indices], y_support[batch_indices]]) # label of support set
                y_mix = torch.concat([y_support[batch_indices], shuffled_y_support[batch_indices]])
 
                if use_fixed_features:
                    f = torch.concat([f_support[batch_indices], f_support_mixed[batch_indices]])
